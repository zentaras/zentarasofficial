...
const STATUS_FILTERS = ["All", "Pending", "Shortlisted", "Rejected", "Today"];
const PROJECT_META = {
  "data-analyst-intern": { icon: "📊", color: "#22a06b" },
  "web-dev-intern":      { icon: "🌐", color: "#6366f1" },
};
...
function ApplicationsTab() {
  ...
  // filtered list — respects project + status + search + today
  const filtered = applicants.filter(a => {
    if (projectFilter !== "all" && a.projectKey !== projectFilter) return false;
    if (statusFilter === "Today") return isToday(a.createdAt);
    if (statusFilter !== "All" && a.status !== statusFilter) return false;
    if (search) {
  ...
  // top stat cards reflect the active project filter (not "all applicants")
  const projectApplicants = projectFilter === "all" ? applicants : applicants.filter(a => a.projectKey === projectFilter);
  const counts = {
    total:       projectApplicants.length,
    pending:     projectApplicants.filter(a => a.status === "Pending").length,
    shortlisted: projectApplicants.filter(a => a.status === "Shortlisted").length,
    rejected:    projectApplicants.filter(a => a.status === "Rejected").length,
    today:       projectApplicants.filter(a => isToday(a.createdAt)).length,
  };

  // per-project breakdown (always uses full applicants list, unaffected by filters)
  const projBreakdown = [
    { key: "data-analyst-intern", icon: "📊", label: "Data Analyst Intern" },
    { key: "web-dev-intern",      icon: "🌐", label: "Web Dev Intern" },
  ].map(p => {
    const sub = applicants.filter(a => a.projectKey === p.key);
    return {
      ...p,
      total:       sub.length,
      pending:     sub.filter(a => a.status === "Pending").length,
      shortlisted: sub.filter(a => a.status === "Shortlisted").length,
      rejected:    sub.filter(a => a.status === "Rejected").length,
      today:       sub.filter(a => isToday(a.createdAt)).length,
    };
  });

  // today count for the "Today" pill badge (respects project filter)
  const todayCount = projectApplicants.filter(a => isToday(a.createdAt)).length;
  ...
  return (
    <div>
      {/* ── Top stat cards (project-filter aware) ── */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 16 }}>
        {[
          ...
          { label: "Today",       val: counts.today,       color: "var(--yellow)" },
        ].map(s => (
          <div key={s.label} style={{ background: "var(--bg-card)", border: `1px solid ${s.label === "Today" && s.val > 0 ? "rgba(226,178,3,0.35)" : "var(--border)"}`, borderRadius: "var(--radius)", padding: "16px 18px" }}>
            <div style={{ fontFamily: "sans-serif", fontSize: 26, fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.val}</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4, textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.label}</div>
          </div>
        ))}
      ...
      {/* ── Per-project breakdown strip ── */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        {projBreakdown.map(p => (
          <div key={p.key} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "12px 16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
              <span style={{ fontSize: 16 }}>{p.icon}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.4px" }}>{p.label}</span>
              {p.today > 0 && (
                <span style={{ fontSize: 10, fontWeight: 700, padding: "1px 7px", borderRadius: 99, background: "var(--yellow-dim)", color: "var(--yellow)", border: "1px solid rgba(226,178,3,0.3)", marginLeft: "auto" }}>+{p.today} today</span>
              )}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>
              {[
                { label: "Total",       val: p.total,       color: "var(--text-primary)" },
                { label: "Pending",     val: p.pending,     color: "var(--blue)" },
                { label: "Shortlisted", val: p.shortlisted, color: "var(--green)" },
                { label: "Rejected",    val: p.rejected,    color: "var(--red)" },
              ].map(s => (
                <div key={s.label} style={{ textAlign: "center", padding: "6px 4px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)" }}>
                  <div style={{ fontSize: 20, fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.val}</div>
                  <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 3, textTransform: "uppercase", letterSpacing: "0.4px" }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "14px 20px", marginBottom: 16, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        <input
          ...
          style={{ flex: 1, minWidth: 200, background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "7px 12px", color: "var(--text-primary)", fontSize: 13, outline: "none", fontFamily: "DM Sans, sans-serif" }}
        />
        <select
          value={projectFilter}
          onChange={e => { setProjectFilter(e.target.value); setStatusFilter("All"); }}
          style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "7px 12px", color: "var(--text-secondary)", fontSize: 12, outline: "none", cursor: "pointer" }}
        >
          {PROJECT_FILTERS.map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
        </select>
            ...
            <button
              ...
              style={{
                ...
                border: statusFilter === s
                  ? (s === "Today" ? "1px solid rgba(226,178,3,0.5)" : "1px solid var(--accent-border)")
                  : "1px solid var(--border)",
                background: statusFilter === s
                  ? (s === "Today" ? "var(--yellow-dim)" : "var(--accent-dim)")
                  : "var(--bg)",
                color: statusFilter === s
                  ? (s === "Today" ? "var(--yellow)" : "var(--accent)")
                  : "var(--text-muted)",
                display: "flex", alignItems: "center", gap: 5,
              }}
            >
              {s}
              {s === "Today" && todayCount > 0 && (
                <span style={{ fontSize: 10, fontWeight: 700, padding: "0px 5px", borderRadius: 99, background: statusFilter === "Today" ? "rgba(226,178,3,0.3)" : "var(--yellow-dim)", color: "var(--yellow)", border: "1px solid rgba(226,178,3,0.3)" }}>
                  {todayCount}
                </span>
              )}
            </button>
      ...
      {/* ── Result count line ── */}
      <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12, paddingLeft: 2 }}>
        Showing {filtered.length} applicant{filtered.length !== 1 ? "s" : ""}
        {statusFilter === "Today" ? " applied today" : statusFilter !== "All" ? ` · ${statusFilter}` : ""}
        {projectFilter !== "all" ? ` · ${PROJECT_FILTERS.find(f => f.key === projectFilter)?.label}` : ""}
      </p>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {filtered.map(app => {
                    ...
                    const appToday = isToday(app.createdAt);
                    return (
                      <tr key={app.id} style={{ borderBottom: "1px solid var(--border)", transition: "background 0.1s" }} onMouseEnter={e => e.currentTarget.style.background = "var(--bg-hover)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        ...
                        <td style={{ padding: "12px 14px", color: "var(--text-muted)", fontSize: 12, whiteSpace: "nowrap" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            ...
                            {appToday && (
                              <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 5px", borderRadius: 99, background: "var(--yellow-dim)", color: "var(--yellow)", border: "1px solid rgba(226,178,3,0.3)", whiteSpace: "nowrap" }}>NEW</span>
                            )}
                          </div>
                        </td>
                        <td style={{ padding: "12px 14px" }}>
                          <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: sc.bg, color: sc.color }}>{app.status}</span>
                        </td>
                        <td style={{ padding: "12px 14px" }}>
                          <div style={{ display: "flex", gap: 6 }}>
                            ...
                            {app.status !== "Shortlisted" && (
                              <button onClick={() => { setReviewModal({ applicant: app, action: "approve" }); setNote(""); }} style={{ padding: "4px 10px", borderRadius: "var(--radius-sm)", fontSize: 11, cursor: "pointer", background: "var(--green-dim)", color: "var(--green)", border: "1px solid rgba(34,160,107,0.3)", fontFamily: "DM Sans, sans-serif", fontWeight: 600 }}>✓ Approve</button>
                            )}
                            {app.status !== "Rejected" && (
                              <button onClick={() => { setReviewModal({ applicant: app, action: "reject" }); setNote(""); }} style={{ padding: "4px 10px", borderRadius: "var(--radius-sm)", fontSize: 11, cursor: "pointer", background: "var(--red-dim)", color: "var(--red)", border: "1px solid rgba(227,73,53,0.25)", fontFamily: "DM Sans, sans-serif", fontWeight: 600 }}>✕ Reject</button>
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setSelected(null)}>
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 10, width: "100%", maxWidth: 640, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: "20px 24px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <h2 style={{ fontFamily: "sans-serif", fontSize: 17, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>{selected.fullName}</h2>
            ...
            <div style={{ padding: "16px 24px 24px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 24px", marginBottom: 16 }}>
                {[["Email",selected.email],["College",selected.college],["Branch",selected.branch],["Graduation",selected.graduationYear],["CGPA",selected.cgpa||"—"]].map(([l,v]) => (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", zIndex: 110, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setReviewModal(null)}>
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 10, width: "100%", maxWidth: 440, boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: "20px 24px 0" }}>
              <h2 style={{ fontFamily: "sans-serif", fontSize: 16, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>{reviewModal.action === "approve" ? "✓ Approve Applicant" : "✕ Reject Applicant"}</h2>
            ...
            <div style={{ padding: "16px 24px 24px" }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>
                {reviewModal.action === "reject" ? "Rejection Reason (required)" : "Note (optional)"}
              </label>
              <textarea value={note} onChange={e => setNote(e.target.value)} rows={4} style={{ width: "100%", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "10px 12px", color: "var(--text-primary)", fontSize: 13, fontFamily: "DM Sans, sans-serif", outline: "none", resize: "vertical", lineHeight: 1.6 }} />
              <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                <button onClick={() => setReviewModal(null)} style={{ flex: 1, padding: "9px 0", background: "transparent", color: "var(--text-secondary)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: 13, cursor: "pointer" }}>Cancel</button>
              ...
              {reviewModal.action === "reject" && !note.trim() && <p style={{ fontSize: 11, color: "var(--red)", marginTop: 6 }}>Rejection reason required.</p>}
            </div>
          </div>
        </div>
      )}
