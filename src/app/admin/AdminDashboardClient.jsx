...
function timeRemaining(deadline, stepStatus) {
  // ✅ Don't show timer when step is approved
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(4px)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onCancel}>
      <div style={{ background: "var(--bg-card)", border: `1px solid ${config.danger ? "rgba(227,73,53,0.45)" : "var(--accent-border)"}`, borderRadius: 12, width: "100%", maxWidth: 400, boxShadow: "0 24px 64px rgba(0,0,0,0.55)", padding: "28px 24px 22px" }} onClick={e => e.stopPropagation()}>
        ...
        <h3 style={{ fontFamily: "Syne, sans-serif", fontSize: 16, fontWeight: 800, color: "var(--text-primary)", marginBottom: 8, textAlign: "center" }}>{config.title}</h3>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={onConfirm} style={{ flex: 2, padding: "9px 0", background: config.danger ? "var(--red)" : "var(--accent)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Syne, sans-serif" }}>{config.confirmLabel}</button>
        </div>
      </div>
    </div>
  );
...
function DeadlineEditor({ stepConfig, stepData, trackId, onDeadlineUpdate, busy }) {
  ...
  // ✅ Don't show timer when approved
  ...
  // ✅ When approved, only show the deadline date (no timer, no edit button)
...
// ── Lock/Unlock toggle ─────────────────────────────────────────────────────────
function StepToggleButton({ stepConfig, isUnlocked, trackId, onToggle, busy }) {
...
// ── Step 5 Evaluation Form (admin fills this) ─────────────────────────────────
function Step5EvaluationRow({ track, trackId, onEvaluate, onToggle, busy }) {
  ...
  const [rating, setRating]               = useState(track.performanceRating ?? "");
  const [feedback, setFeedback]           = useState(track.overallFeedback ?? "");
  ...
  const hasEvaluation = !!(track.performanceRating && track.overallFeedback);
  ...
  const handleSave = async (publish = false) => {
    ...
    await onEvaluate({
      ...
      // Only mark completed if publish=true and both required fields filled
      ...(publish && rating && feedback ? {} : {}),
    }, publish);
    setSaving(false);
    setSaved(true);
  ...
  const handlePublish = () => {
    if (!rating) { alert("Please select a performance rating first."); return; }
    if (!feedback.trim()) { alert("Please add overall feedback first."); return; }
    setConfirm({
      icon: "🎓", danger: false,
      title: "Publish Evaluation?",
      message: "This will mark the internship as completed and make the evaluation visible to the intern.",
      warning: certLink ? `Certificate link: ${certLink}` : "No certificate link set — you can add one after publishing.",
      ...
      <div onClick={() => isUnlocked && setOpen(v => !v)} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 16px", cursor: isUnlocked ? "pointer" : "default", borderRadius: "var(--radius-sm)", background: !isUnlocked ? "rgba(0,0,0,0.06)" : "transparent" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: !isUnlocked ? "var(--text-muted)" : "var(--text-primary)" }}>
              ...
              {!step4Done && isUnlocked && (
                <span style={{ marginLeft: 8, fontSize: 10, background: "var(--yellow-dim)", color: "var(--yellow)", padding: "1px 7px", borderRadius: 3, fontWeight: 700 }}>
                  ⚠ Step 4 not yet approved
                </span>
              )}
            </p>
            <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 10px", borderRadius: 12, textTransform: "uppercase", letterSpacing: "0.4px", whiteSpace: "nowrap", background: isCompleted ? "var(--green-dim)" : isUnlocked ? "var(--accent-dim)" : STEP_STATUS_COLORS.pending.bg, color: isCompleted ? "var(--green)" : isUnlocked ? "var(--accent)" : STEP_STATUS_COLORS.pending.color, border: `1px solid ${isCompleted ? "rgba(34,160,107,0.3)" : isUnlocked ? "var(--accent-border)" : STEP_STATUS_COLORS.pending.border}` }}>
              {isCompleted ? "Published ✓" : isUnlocked ? (hasEvaluation ? "Evaluation Saved (Draft)" : "Admin Action Required") : "🔒 Locked"}
            </span>
          ...
          {isUnlocked && !isCompleted && (
            <p style={{ fontSize: 11, color: "var(--text-muted)" }}>
              {hasEvaluation ? "Draft saved — click Publish to make visible to intern." : "Fill in performance evaluation and publish to complete the internship."}
            </p>
          )}
          {isCompleted && (
            <p style={{ fontSize: 11, color: "var(--green)" }}>✓ Evaluation published · Internship completed</p>
          )}
        </div>
      ...
      {open && isUnlocked && (
        <div style={{ padding: "16px 18px", borderTop: "1px solid var(--border)", background: "var(--bg)" }}>
          ...
          <div style={{ background: "var(--accent-dim)", border: "1px solid var(--accent-border)", borderRadius: "var(--radius-sm)", padding: "10px 14px", marginBottom: 18, fontSize: 12, color: "var(--accent)" }}>
            🎓 <strong>Admin fills this section.</strong> The intern does not submit anything here — they see your evaluation once you publish it.
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "var(--text-muted)", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.5px" }}>
              Overall Feedback for Intern <span style={{ color: "var(--red)" }}>*</span>
            </label>
            <textarea value={feedback} onChange={e => setFeedback(e.target.value)} rows={4} placeholder="Write detailed performance feedback — strengths, areas of improvement, and overall assessment..." style={{ width: "100%", boxSizing: "border-box", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "9px 12px", color: "var(--text-primary)", fontSize: 12, fontFamily: "DM Sans, sans-serif", outline: "none", resize: "vertical", lineHeight: 1.6 }} />
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 14px", marginBottom: 14 }}>
            <div>
              ...
              <input type="url" value={certLink} onChange={e => setCertLink(e.target.value)} placeholder="https://drive.google.com/... or certificate URL" style={{ width: "100%", boxSizing: "border-box", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "8px 10px", color: "var(--text-primary)", fontSize: 12, fontFamily: "DM Sans, sans-serif", outline: "none" }} />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: "var(--text-muted)", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.5px" }}>Letter of Recommendation Link</label>
              <input type="url" value={lorLink} onChange={e => setLorLink(e.target.value)} placeholder="https://drive.google.com/... LOR link" style={{ width: "100%", boxSizing: "border-box", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "8px 10px", color: "var(--text-primary)", fontSize: 12, fontFamily: "DM Sans, sans-serif", outline: "none" }} />
            </div>
          </div>
          ...
          {/* Internal evaluation notes (not shown to intern) */}
          ...
          {/* Actions */}
          {saved && <p style={{ fontSize: 12, color: "var(--green)", marginBottom: 10 }}>✓ Saved as draft.</p>}
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            {!isCompleted && (
              <button onClick={handlePublish} disabled={saving} style={{ padding: "8px 24px", background: "var(--green)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Syne, sans-serif", opacity: saving ? 0.7 : 1 }}>
...
function StepRow({ stepConfig, stepData, isUnlocked, isCurrentlyActive, trackId, onApproveReject, onToggle, onDeadlineUpdate, busy }) {
  ...
  // ✅ Don't show timer when approved
  const remaining = timeRemaining(deadline, status);
    ...
    <div style={{ border: `1px solid ${stepData?.status === "submitted" ? "rgba(226,178,3,0.5)" : "var(--border)"}`, borderRadius: "var(--radius-sm)", marginBottom: 10, overflow: "visible" }}>
      <div onClick={() => isUnlocked && setOpen(v => !v)} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 16px", cursor: isUnlocked ? "pointer" : "default", borderRadius: "var(--radius-sm)", background: !isUnlocked ? "rgba(0,0,0,0.06)" : stepData?.status === "submitted" ? "rgba(226,178,3,0.04)" : "transparent" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* ✅ Deadline shows but no timer when approved */}
        ...
        <div style={{ padding: "14px 16px", borderTop: "1px solid var(--border)", background: "var(--bg)" }}>
          {(!stepData || status === "pending") ? (
            ...
            <>
              {stepData.data && Object.entries(stepData.data).map(([k, v]) => v ? (
                <div key={k} style={{ marginBottom: 12 }}>
                  ...
                  {(k.toLowerCase().includes("link") || k.toLowerCase().includes("url"))
                    ...
                    : <div style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.6, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "8px 10px", maxHeight: 120, overflowY: "auto" }}>{v}</div>}
                </div>
              ) : null)}
              {stepData.adminNote && (
              ...
              {status === "submitted" && (
                <div style={{ marginTop: 8 }}>
                  <div style={{ background: "var(--bg-card)", border: "1px solid var(--accent-border)", borderRadius: "var(--radius-sm)", padding: "8px 12px", marginBottom: 10, fontSize: 12, color: "var(--accent)" }}>
                    💡 After approving, <strong>manually unlock the next step</strong> — it does not happen automatically.
                  </div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", display: "block", marginBottom: 5 }}>Feedback</label>
                  <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Leave feedback…" rows={3} style={{ width: "100%", boxSizing: "border-box", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "8px 10px", color: "var(--text-primary)", fontSize: 12, fontFamily: "DM Sans, sans-serif", outline: "none", resize: "vertical", lineHeight: 1.5 }} />
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <button onClick={() => { onApproveReject(stepData.id, "approve", note); setOpen(false); }} style={{ padding: "7px 20px", background: "var(--green)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "Syne, sans-serif" }}>✓ Approve Step {stepConfig.number}</button>
                    <button onClick={() => { if (!note.trim()) { alert("Add rejection reason first."); return; } onApproveReject(stepData.id, "reject", note); setOpen(false); }} style={{ padding: "7px 20px", background: "var(--red-dim)", color: "var(--red)", border: "1px solid rgba(227,73,53,0.3)", borderRadius: "var(--radius-sm)", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "DM Sans, sans-serif" }}>✕ Request Revision</button>
                  </div>
                </div>
              )}
            </>
...
// ── Track Modal ────────────────────────────────────────────────────────────────
function TrackDetailModal({ track, onClose, onStepAction, onToggleStep, onDeadlineUpdate, onEvaluate }) {
        ...
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: 16, fontWeight: 800, color: "var(--text-primary)", marginBottom: 2 }}>{track.user?.firstName ?? ""} {track.user?.lastName ?? ""}</h2>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 14 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: track.isCompleted ? "var(--green)" : "var(--accent)", whiteSpace: "nowrap" }}>{track.isCompleted ? "✓ Completed" : `${progress}% · Step ${track.currentStep} active`}</span>
          </div>
          <div style={{ marginTop: 12, padding: "10px 14px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.6 }}>
            <strong style={{ color: "var(--accent)" }}>Admin controls:</strong>{" "}
            🔓 Unlock / 🔒 Lock any step · 📅 Edit deadlines · ✅ Approve/reject submissions · Step 5 = <strong>admin fills evaluation</strong>, not candidate.
          </div>
        </div>

        <div style={{ overflowY: "auto", padding: "16px 24px 24px", flex: 1 }}>
          <p style={{ fontFamily: "Syne, sans-serif", fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 14 }}>All Steps — Full Control</p>

          {/* Steps 1–4 */}
          {INTERNSHIP_STEPS.filter(cfg => cfg.number !== 5).map(cfg => {
            const stepData       = track.steps.find(s => s.stepNumber === cfg.number) ?? null;
            ...
            return (
              <StepRow
                ...
                trackId={track.id}
                onApproveReject={(stepId, action, note) => onStepAction(stepId, action, note, track.id)}
...
function InternshipTracksTab() {
  ...
  const handleStepAction = async (stepId, action, note, trackId) => {
    await fetch("/api/admin/tracks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stepId, action, adminNote: note }) });
    await fetchTracks();
    if (selectedTrack?.id === trackId) await refreshModal(trackId);
  };
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
        {[{ label: "Total Interns", val: counts.total, color: "var(--text-primary)" }, { label: "Active", val: counts.active, color: "var(--accent)" }, { label: "Completed", val: counts.completed, color: "var(--green)" }, { label: "Awaiting Review", val: counts.pendingReview, color: "var(--yellow)" }].map(s => (
          <div key={s.label} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "16px 18px" }}>
            <div style={{ fontFamily: "Syne, sans-serif", fontSize: 26, fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.val}</div>
            <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 4, textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.label}</div>
          </div>
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "14px 20px", marginBottom: 16, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>{[{ key: "all", label: "All" }, { key: "active", label: "Active" }, { key: "completed", label: "Completed" }].map(s => (<button key={s.key} onClick={() => setStatusFilter(s.key)} style={{ padding: "5px 12px", borderRadius: "var(--radius-sm)", fontSize: 12, cursor: "pointer", fontFamily: "DM Sans, sans-serif", fontWeight: statusFilter === s.key ? 700 : 400, border: statusFilter === s.key ? "1px solid var(--accent-border)" : "1px solid var(--border)", background: statusFilter === s.key ? "var(--accent-dim)" : "var(--bg)", color: statusFilter === s.key ? "var(--accent)" : "var(--text-muted)" }}>{s.label}</button>))}</div>
      </div>
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden" }}>
        {loading ? <div style={{ padding: 48, textAlign: "center", color: "var(--text-muted)" }}>Loading…</div>
        : filtered.length === 0 ? <div style={{ padding: 48, textAlign: "center", color: "var(--text-muted)" }}><div style={{ fontSize: 32, marginBottom: 10 }}>📭</div><p>No tracks yet.</p></div>
        : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead style={{ background: "var(--bg-hover)" }}><tr>{["Intern","Project","Step Progress","Review","Status","Actions"].map(h => (<th key={h} style={{ padding: "10px 14px", textAlign: "left", fontFamily: "Syne, sans-serif", fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.7px", borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>{h}</th>))}</tr></thead>
              <tbody>
                {filtered.map(track => {
                  const meta = PROJECT_META[track.projectKey] ?? { icon: "🎓", color: "#6366f1" };
                  const approvedCount = track.steps.filter(s => s.status === "approved").length;
                  ...
                  const needsEval     = track.currentStep >= 5 && !track.isCompleted && !track.performanceRating;
                  return (
                    <tr key={track.id} style={{ borderBottom: "1px solid var(--border)", transition: "background 0.1s" }} onMouseEnter={e => e.currentTarget.style.background = "var(--bg-hover)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                      <td style={{ padding: "12px 14px" }}><p style={{ fontWeight: 600, color: "var(--text-primary)", marginBottom: 2 }}>{track.user?.firstName ?? ""} {track.user?.lastName ?? ""}</p><p style={{ fontSize: 11, color: "var(--text-muted)" }}>{track.user?.email}</p></td>
                      <td style={{ padding: "12px 14px" }}><div style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ fontSize: 15 }}>{meta.icon}</span><span style={{ fontSize: 12, color: "var(--text-secondary)" }}>{track.projectName}</span></div></td>
                      <td style={{ padding: "12px 14px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ display: "flex", gap: 3 }}>{INTERNSHIP_STEPS.map(cfg => { const s = track.steps.find(st => st.stepNumber === cfg.number); const status = s?.status ?? (cfg.number <= track.currentStep ? "pending" : "locked"); return <div key={cfg.number} title={`Step ${cfg.number}: ${status}`} style={{ width: 11, height: 11, borderRadius: "50%", background: status === "approved" ? "var(--green)" : status === "submitted" ? "var(--yellow)" : status === "rejected" ? "var(--red)" : status === "pending" ? "var(--accent)" : "var(--border)" }} />; })}</div>
                      ...
                      <td style={{ padding: "12px 14px" }}><span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: track.isCompleted ? "var(--green-dim)" : "var(--accent-dim)", color: track.isCompleted ? "var(--green)" : "var(--accent)", border: `1px solid ${track.isCompleted ? "rgba(34,160,107,0.3)" : "var(--accent-border)"}` }}>{track.isCompleted ? "✓ Done" : `Step ${track.currentStep} Active`}</span></td>
                      <td style={{ padding: "12px 14px" }}>
...
export default function AdminDashboardClient() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text-primary)" }}>
      ...
      <main style={{ maxWidth: 1140, margin: "0 auto", padding: "32px 20px 80px" }}>
        <div style={{ marginBottom: 24 }}><h1 style={{ fontFamily: "Syne, sans-serif", fontSize: 22, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>Admin Dashboard</h1><p style={{ fontSize: 13, color: "var(--text-muted)" }}>Manage applicants and internship progress</p></div>
        <div style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: "1px solid var(--border)" }}>
          {[{ key: "applications", label: "📥 Applications" }, { key: "internships", label: "🎓 Internship Tracks" }].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{ padding: "10px 20px", borderRadius: "var(--radius-sm) var(--radius-sm) 0 0", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "DM Sans, sans-serif", border: "1px solid var(--border)", borderBottom: tab === t.key ? "2px solid var(--accent)" : "1px solid transparent", background: tab === t.key ? "var(--bg-card)" : "transparent", color: tab === t.key ? "var(--accent)" : "var(--text-muted)", marginBottom: -1, transition: "all 0.15s" }}>{t.label}</button>
          ))}
