...
// ─── Step 1 Briefing Panel (admin-controlled, intern read-only) ────────────────
        ...
        {fields.map(({ key, label, icon }) => briefing[key] ? (
          <div key={key} style={{ marginBottom: 16 }}>
            ...
            <div style={{
              ...
              padding: "10px 14px", fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.7, whiteSpace: "pre-wrap",
            }}>
              {briefing[key]}
            </div>
          </div>
...
function Step5CandidatePanel({ track }) {
  ...
  // Calculate total points across steps 1–4
            ...
            {[1, 2, 3, 4].map(n => {
              ...
              return (
                <div key={n} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)" }}>
                  ...
                  <span style={{ fontSize: 12, color: "var(--text-primary)", flex: 1 }}>{config?.title}</span>
                  {pts != null
                    ? <PointsBadge points={pts} size="small" />
      ...
      {Array.from({ length: totalSteps }).map((_, i) => {
        ...
        return (
          <div key={sNum} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
            ...
            <div style={{ paddingTop: 6, paddingBottom: sNum < totalSteps ? 24 : 0, flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: 11, fontWeight: isActive ? 700 : 500, lineHeight: 1.3, marginBottom: 3, color: isLocked ? "var(--text-muted)" : isActive ? "var(--accent)" : "var(--text-primary)" }}>
                {INTERNSHIP_STEPS[i]?.title}
              </p>
              {/* Points in sidebar */}
              {points != null && (
                <PointsBadge points={points} size="small" />
              )}
              {showDeadlineBadge && deadlineRemaining && (
                <span style={{
      ...
      <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", background: isActive ? "var(--bg)" : "transparent" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", flexShrink: 0, alignItems: "center" }}>
            {/* Points badge — shown once admin awards points */}
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: isMobile ? "16px 12px 80px" : "28px 20px 80px" }}>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: isMobile ? "16px" : "20px 24px", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <h1 style={{ fontFamily: "sans-serif", fontSize: isMobile ? 15 : 18, fontWeight: 800, color: "var(--text-primary)", marginBottom: 2 }}>
